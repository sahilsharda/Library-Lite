...
router.get("/user/:userId", authenticateToken, async (req, res) => {
  try {
    ...
    const activeBorrows = enrichedLoans.filter(
      (l) => l.status === "borrowed",
    ).length;
    const overdueBorrows = enrichedLoans.filter(
      (l) => l.status === "overdue",
    ).length;
    const returnedBorrows = enrichedLoans.filter(
      (l) => l.status === "returned",
    ).length;
    ...
    const walletBalance =
      (member?.membership?.initialBalance || 0) - totalFines + paidFines;
